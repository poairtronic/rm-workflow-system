...
describe('InventoryReconciliationService & InventoryService - Phase 10.6 Verification', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: getRepositoryToken(StockBalance), useValue: { create: vi.fn() } },
        { provide: getRepositoryToken(StockTransaction), useValue: { create: vi.fn() } },
    ...
    reconciliationService = module.get<InventoryReconciliationService>(InventoryReconciliationService);
  ...
  describe('Certification Tests', () => {
    it('VERIFY-001: Product + Bin is authoritative and StockBalance is not duplicated', async () => {
      manager.findOne.mockResolvedValueOnce({ id: 'item-1' }) // item
      ...
      const result = await reconciliationService.manualMapAndReconcile('item-1', 'prod-1', 'bin-1');
      ...
      expect(manager.update).toHaveBeenCalledWith(StockBalance, { id: 'sb-2' }, expect.any(Object));
    });
    ...
    it('VERIFY-002: InventoryItem remains preserved', async () => {
      ...
      manager.findOne.mockResolvedValueOnce({ id: 'item-1' }) 
             .mockResolvedValueOnce({ id: 'prod-1' }) 
             .mockResolvedValueOnce({ id: 'bin-1' }) 
             .mockResolvedValueOnce({ id: 'sb-1', currentQuantity: 5 }) 
             .mockResolvedValueOnce(null); 

      await reconciliationService.manualMapAndReconcile('item-1', 'prod-1', 'bin-1');
    ...
    it('VERIFY-011: Opening balance is not double-counted (idempotent overwrite)', async () => {
      manager.findOne.mockResolvedValueOnce({ id: 'item-1' }) 
             .mockResolvedValueOnce({ id: 'prod-1' }) 
             .mockResolvedValueOnce({ id: 'bin-1' }) 
             .mockResolvedValueOnce({ id: 'sb-1', currentQuantity: 5, openingBalance: 10 }) 
             .mockResolvedValueOnce(null); 

      await reconciliationService.manualMapAndReconcile('item-1', 'prod-1', 'bin-1');
      
      // Inherits existing balance if not specified
      expect(manager.update).toHaveBeenCalledWith(StockBalance, { id: 'sb-1' }, expect.objectContaining({ openingBalance: 10 }));
    });
    ...
    it('VERIFY-013: Historical transactions remain immutable', async () => {
      ...
      await reconciliationService.manualMapAndReconcile('item-1', 'prod-1', 'bin-1');
      expect(manager.update.mock.calls.some(call => call[0] === StockTransaction)).toBe(false);
    });
    ...
    it('VERIFY-015: Actual reconciliation is idempotent', async () => {
      ...
      const res = await reconciliationService.manualMapAndReconcile('item-1', 'prod-1', 'bin-1');
    ...
    it('VERIFY-016: Rollback works on constraint failure', async () => {
      ...
      await expect(reconciliationService.manualMapAndReconcile('item-1', 'prod-1', 'bin-1')).rejects.toThrow();
      expect(queryRunner.rollbackTransaction).toHaveBeenCalled();
    });
    ...
    it('VERIFY-018: Stock IN propagates Product + Bin correctly', async () => {
      ...
      await inventoryService.stockIn('item-1', { quantity: 5, referenceType: 'PO', referenceId: '123' }, 'user-1');
    ...
    it('VERIFY-019: Stock OUT propagates Product + Bin correctly', async () => {
      ...
      await inventoryService.stockOut('item-1', { quantity: 5, referenceType: 'SC', referenceId: '123' }, 'user-1');
