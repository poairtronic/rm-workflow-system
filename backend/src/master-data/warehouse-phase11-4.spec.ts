...
describe('Phase 11.4 — Warehouse Master Data Specification', () => {
  ...
  describe('Warehouse Creation & Validation', () => {
    it('WAREHOUSE-001: Create warehouse successfully, WAREHOUSE-013: active default', async () => {
      ...
      expect(warehouseRepo.create).toHaveBeenCalledWith(
        expect.objectContaining({ code: 'WH001', name: 'Main Warehouse' }),
      );
    });
    ...
    it('WAREHOUSE-014, WAREHOUSE-016: Reject duplicate code (case-insensitive normalization)', async () => {
      warehouseRepo.createQueryBuilder = vi
        .fn()
        .mockImplementationOnce(() =>
          createMockQueryBuilder({ id: 'wh-ex', code: 'WH001' }),
        );
      await expect(
        service.createWarehouse({ code: 'wh001', name: 'New Name' }),
    ...
    it('WAREHOUSE-015, WAREHOUSE-017: Reject duplicate name (case-insensitive)', async () => {
      ...
      warehouseRepo.createQueryBuilder = vi
        ...
        .mockImplementationOnce(() =>
          createMockQueryBuilder({ id: 'wh-ex', name: 'Main Warehouse' }),
        );

      await expect(
        service.createWarehouse({ code: 'WH002', name: 'main warehouse' }),
      ).rejects.toThrow(ConflictException);
    });
  });
  ...
  describe('Warehouse Reading', () => {
    ...
    it('WAREHOUSE-022: Return not-found for nonexistent warehouse', async () => {
      ...
      await expect(service.findWarehouseById('wh-none')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('Warehouse Updates & Lifecycle', () => {
    it('WAREHOUSE-023: Update warehouse successfully', async () => {
      ...
      const result = await service.updateWarehouse('wh-1', {
    ...
    it('WAREHOUSE-024, WAREHOUSE-025: Reject duplicate code or name during update', async () => {
      ...
      warehouseRepo.createQueryBuilder = vi
        .fn()
        .mockImplementationOnce(() =>
          createMockQueryBuilder({ id: 'wh-2', code: 'WH002' }),
        );
      await expect(
        service.updateWarehouse('wh-1', { code: 'WH002' }),
      ).rejects.toThrow(ConflictException);
      ...
      warehouseRepo.createQueryBuilder = vi
        .fn()
        .mockImplementationOnce(() =>
          createMockQueryBuilder({ id: 'wh-3', name: 'Existing Name' }),
        );
      await expect(
        service.updateWarehouse('wh-1', { name: 'Existing Name' }),
      ).rejects.toThrow(ConflictException);
    });
  ...
  describe('Warehouse Deletion & Dependency Protection', () => {
    it('WAREHOUSE-029: Prevent deletion when dependent WarehouseLocation exists', async () => {
      ...
      await expect(service.deleteWarehouse('wh-1')).rejects.toThrow(
        ConflictException,
      );
    });
