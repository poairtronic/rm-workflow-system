...
describe('Phase 11.8 — Master Data Security', () => {
  ...
  describe('RBAC Verification (SEC-MD-001 to SEC-MD-023)', () => {
    it('SEC-MD-001, 002, 003: Category Create is strictly ADMIN + STORES', () => {
      expectWriteRoles(Reflect.getMetadata(ROLES_KEY, categoriesController.createCategory));
    });

    it('SEC-MD-004, 005, 006: Family Create is strictly ADMIN + STORES', () => {
      expectWriteRoles(Reflect.getMetadata(ROLES_KEY, familiesController.createFamily));
    });

    it('SEC-MD-007, 008, 009: Product Create is strictly ADMIN + STORES', () => {
      expectWriteRoles(Reflect.getMetadata(ROLES_KEY, productsController.createProduct));
    });

    it('SEC-MD-010, 011, 012: Warehouse Create is strictly ADMIN + STORES', () => {
      expectWriteRoles(Reflect.getMetadata(ROLES_KEY, warehousesController.createWarehouse));
    });

    it('SEC-MD-013, 014, 015: Location Create is strictly ADMIN + STORES', () => {
      expectWriteRoles(Reflect.getMetadata(ROLES_KEY, locationsController.createLocation));
    });

    it('SEC-MD-016, 017, 018: Rack Create is strictly ADMIN + STORES', () => {
      expectWriteRoles(Reflect.getMetadata(ROLES_KEY, racksController.createRack));
    });

    it('SEC-MD-019, 020, 021: Bin Create is strictly ADMIN + STORES', () => {
      expectWriteRoles(Reflect.getMetadata(ROLES_KEY, binsController.createBin));
    });

    it('SEC-MD-022: Unauthorized update rejected across master data', () => {
      expectWriteRoles(Reflect.getMetadata(ROLES_KEY, categoriesController.updateCategory));
      expectWriteRoles(Reflect.getMetadata(ROLES_KEY, familiesController.updateFamily));
      expectWriteRoles(Reflect.getMetadata(ROLES_KEY, productsController.updateProduct));
      expectWriteRoles(Reflect.getMetadata(ROLES_KEY, warehousesController.updateWarehouse));
      expectWriteRoles(Reflect.getMetadata(ROLES_KEY, locationsController.updateLocation));
      expectWriteRoles(Reflect.getMetadata(ROLES_KEY, racksController.updateRack));
      expectWriteRoles(Reflect.getMetadata(ROLES_KEY, binsController.updateBin));
    });

    it('SEC-MD-023: Unauthorized delete rejected across master data', () => {
      expectWriteRoles(Reflect.getMetadata(ROLES_KEY, categoriesController.deleteCategory));
      expectWriteRoles(Reflect.getMetadata(ROLES_KEY, familiesController.deleteFamily));
      // Product deletion is explicitly missing per design
      expect(productsController['deleteProduct']).toBeUndefined();
      expectWriteRoles(Reflect.getMetadata(ROLES_KEY, warehousesController.deleteWarehouse));
      expectWriteRoles(Reflect.getMetadata(ROLES_KEY, locationsController.deleteLocation));
      expectWriteRoles(Reflect.getMetadata(ROLES_KEY, racksController.deleteRack));
      expectWriteRoles(Reflect.getMetadata(ROLES_KEY, binsController.deleteBin));
    });
  });
  ...
  describe('Senior Designer & AMR Verification (SEC-MD-050 to SEC-MD-051)', () => {
    it('SEC-MD-050: SENIOR_DESIGNER remains absent from read boundaries', () => {
      expectReadRoles(Reflect.getMetadata(ROLES_KEY, categoriesController.findCategories));
      expectReadRoles(Reflect.getMetadata(ROLES_KEY, familiesController.findFamilies));
      expectReadRoles(Reflect.getMetadata(ROLES_KEY, productsController.findProducts));
      expectReadRoles(Reflect.getMetadata(ROLES_KEY, warehousesController.findWarehouses));
      expectReadRoles(Reflect.getMetadata(ROLES_KEY, locationsController.findLocations));
      expectReadRoles(Reflect.getMetadata(ROLES_KEY, racksController.findRacks));
