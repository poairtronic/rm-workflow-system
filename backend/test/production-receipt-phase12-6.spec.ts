...
describe('Phase 12.6 - Production Receipt', () => {
  beforeAll(async () => {
    ...
    const custRes = await fetch(`${BASE_URL}/customers`, {
      ...
      body: JSON.stringify({ name: 'Prod Customer', code: `PRD-${Date.now()}`, isActive: true })
    });
    ...
    const itemRes = await fetch(`${BASE_URL}/rm/${rmId}/items`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${adminToken}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ material: 'Aluminium', grade: '6061-T6', size: '10x10', quantity: 50 })
    });
    ...
    const p1 = await pgClient.query(`INSERT INTO products (name, family_id, minimum_inventory) VALUES ('Product Receipt ' || gen_random_uuid()::text, '${fam.rows[0].id}', 0) RETURNING id`);
  ...
  it('RECEIPT_03: Should correctly create partial receipt WITHOUT second stock deduction', async () => {
    const stockBefore = await pgClient.query(`SELECT current_quantity FROM stock_balances WHERE product_id = '${productId}'`);
    ...
    const stockAfter = await pgClient.query(`SELECT current_quantity FROM stock_balances WHERE product_id = '${productId}'`);
