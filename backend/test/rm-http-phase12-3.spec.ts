...
describe('Phase 12.3 - RM Request Real HTTP / Inventory Isolation Verification', () => {
  beforeAll(async () => {
    ...
    adminToken = jwt.sign(payload, process.env.JWT_SECRET || 'your_development_jwt_secret_min_32_characters');

    // 1. Setup Customer & PO
    const custRes = await fetch(`${BASE_URL}/customers`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${adminToken}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ name: 'RM Test Customer', code: `RM-CUST-${Date.now()}`, isActive: true })
    });
    ...
    const scRes1 = await fetch(`${BASE_URL}/sc`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${adminToken}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ poId: poId, scNumber: `SC001`, productName: 'Widget A', targetQuantity: 10 })
    });
    ...
    const scRes2 = await fetch(`${BASE_URL}/sc`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${adminToken}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ poId: poId, scNumber: `SC002`, productName: 'Widget B', targetQuantity: 20 })
    });
  ...
  it('RM_VALIDATION_01: Should block RM creation for missing/invalid SC', async () => {
    ...
    const res1 = await fetch(`${BASE_URL}/rm`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${adminToken}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ scId: '00000000-0000-0000-0000-000000000000', remarks: 'Test' })
    });
    ...
    const res2 = await fetch(`${BASE_URL}/rm`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${adminToken}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ scId: '11111111-1111-4111-a111-111111111111', remarks: 'Test' })
    });
  ...
  it('RM_SUBMISSION_01: Should submit RM001 successfully and update SC status', async () => {
    ...
    const scCheckBefore = await fetch(`${BASE_URL}/sc/${scId1}`, { headers: { 'Authorization': `Bearer ${adminToken}` } });
    ...
    const scCheckAfter = await fetch(`${BASE_URL}/sc/${scId1}`, { headers: { 'Authorization': `Bearer ${adminToken}` } });
    expect((await scCheckAfter.json()).status).toBe('SUBMITTED');

    // Verify SC002 remains unchanged
    const sc2Check = await fetch(`${BASE_URL}/sc/${scId2}`, { headers: { 'Authorization': `Bearer ${adminToken}` } });
  ...
  it('RM_VALIDATION_04: Cannot add items to SUBMITTED RM', async () => {
    const res = await fetch(`${BASE_URL}/rm/${rmId1}/items`, {
      ...
      headers: { 'Authorization': `Bearer ${adminToken}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({
        material: 'Copper', grade: 'C101', quantity: 10, size: '10mm'
      })
    });
  ...
  it('INVENTORY_ISOLATION_01: RM Creation and Submission must NOT mutate Stock', async () => {
    const query = await pgClient.query(`SELECT COUNT(*) as count FROM stock_transactions`);
    expect(query.rows[0].count).toBeDefined();
    
    // We can confidently assert `materialIssues` on RM Item is 0
    const rmVerify = await fetch(`${BASE_URL}/rm/${rmId1}`, { headers: { 'Authorization': `Bearer ${adminToken}` } });
