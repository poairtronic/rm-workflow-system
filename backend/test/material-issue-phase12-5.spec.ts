...
describe('Phase 12.5 - Stores Material Issue', () => {
  beforeAll(async () => {
    ...
    const custRes = await fetch(`${BASE_URL}/customers`, {
      ...
      body: JSON.stringify({ name: 'Issue Customer', code: `ISS-CUST-${Date.now()}`, isActive: true })
    });
    ...
    const scRes = await fetch(`${BASE_URL}/sc`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${adminToken}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ poId: poId, scNumber: `SC-ISS-01`, productName: 'Widget Issue', targetQuantity: 10 })
    });
    ...
    const itemRes = await fetch(`${BASE_URL}/rm/${rmId1}/items`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${adminToken}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ material: 'Aluminium', grade: '6061', quantity: 50, size: '20mm' })
    });
    ...
    const p1 = await pgClient.query(`INSERT INTO products (name, family_id, minimum_inventory) VALUES ('Product Alu 6061 20mm ' || gen_random_uuid()::text, '${famId}', 0) RETURNING id`);
  ...
  it('ISSUE_01: Should safely reject issue for unmapped product or DRAFT RM', async () => {
    const scDraft = await fetch(`${BASE_URL}/sc`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${adminToken}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ poId: poId, scNumber: `SC-ISS-02`, productName: 'Widget DRAFT', targetQuantity: 10 })
    });
  ...
  it('ISSUE_02: Should successfully physically issue stock and create atomic transaction', async () => {
    const balBefore = await pgClient.query(`SELECT current_quantity FROM stock_balances WHERE product_id = '${productId1}' AND bin_id = '${binId}'`);
    ...
    const balAfter = await pgClient.query(`SELECT current_quantity FROM stock_balances WHERE product_id = '${productId1}' AND bin_id = '${binId}'`);
