...
describe('Phase 12.7 - Production Consumption', () => {
  beforeAll(async () => {
    ...
    const p1 = await pgClient.query(
      `INSERT INTO products (name, family_id, minimum_inventory) VALUES ('Product Consume ' || gen_random_uuid()::text, '${fam.rows[0].id}', 0) RETURNING id`,
    );
  ...
  it('CONSUME_05: Concurrency - Should enforce total consumed limit under concurrent load', async () => {
    ...
    const accRes = await fetch(`${BASE_URL}/production/accounting/${scId}`, {
      headers: { Authorization: `Bearer ${prodToken}` },
    });
