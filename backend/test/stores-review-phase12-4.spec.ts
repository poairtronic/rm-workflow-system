...
describe('Phase 12.4 - Stores Review / Inventory Verification', () => {
  beforeAll(async () => {
    ...
    const p1 = await pgClient.query(
      `INSERT INTO products (name, family_id, minimum_inventory) VALUES ('Product Steel 304 10mm ' || gen_random_uuid()::text, '${famId}', 0) RETURNING id`,
    );
    const p2 = await pgClient.query(
      `INSERT INTO products (name, family_id, minimum_inventory) VALUES ('Product Copper C101 5mm ' || gen_random_uuid()::text, '${famId}', 0) RETURNING id`,
    );
    ...
    await pgClient.query(
      `INSERT INTO stock_balances (product_id, bin_id, current_quantity) VALUES ('${productId2}', '${binId}', 150)`,
    );
  });
  ...
  it('REVIEW_01: Should perform Stores Review and correctly classify item availability without mutating physical stock', async () => {
    ...
    const bal1 = await pgClient.query(
      `SELECT current_quantity FROM stock_balances WHERE product_id = '${productId1}'`,
    );
