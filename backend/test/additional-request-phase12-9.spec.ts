...
describe('Phase 12.9 - Additional Material Request', () => {
  ...
  beforeAll(async () => {
    pgClient = new Client({
      connectionString: process.env.DATABASE_URL || 'postgres://postgres:postgres@localhost:5432/rm_workflow_db',
    });
    ...
    adminToken = jwt.sign({ sub: EXISTING_USER_ID, role: 'ADMIN', roles: ['ADMIN'] }, secret);
    prodToken = jwt.sign({ sub: EXISTING_USER_ID, role: 'PRODUCTION', roles: ['PRODUCTION'] }, secret);
    storesToken = jwt.sign({ sub: EXISTING_USER_ID, role: 'STORES', roles: ['STORES'] }, secret);

    // 2. Setup master data and flow
    const timestamp = Date.now();
    const catRes = await pgClient.query(`INSERT INTO product_categories (name, is_active) VALUES ('ADDL_CAT_${timestamp}', true) RETURNING id`);
    const famRes = await pgClient.query(`INSERT INTO product_families (name, category_id, is_active) VALUES ('ADDL_FAM_${timestamp}', '${catRes.rows[0].id}', true) RETURNING id`);
    const prodResDb = await pgClient.query(`INSERT INTO products (name, family_id, is_active) VALUES ('ADDL_PROD_${timestamp}', '${famRes.rows[0].id}', true) RETURNING id`);
    productId = prodResDb.rows[0].id;

    const whRes = await pgClient.query(`INSERT INTO warehouses (name, code, is_active) VALUES ('ADDL_WH_${timestamp}', 'AWH_${timestamp}', true) RETURNING id`);
    const locRes = await pgClient.query(`INSERT INTO warehouse_locations (name, code, warehouse_id, is_active) VALUES ('ADDL_LOC_${timestamp}', 'ALOC_${timestamp}', '${whRes.rows[0].id}', true) RETURNING id`);
    const rackRes = await pgClient.query(`INSERT INTO racks (name, code, location_id, is_active) VALUES ('ADDL_RACK_${timestamp}', 'ARACK_${timestamp}', '${locRes.rows[0].id}', true) RETURNING id`);
    const binResDb = await pgClient.query(`INSERT INTO bins (name, code, rack_id, is_active) VALUES ('ADDL_BIN_${timestamp}', 'ABIN_${timestamp}', '${rackRes.rows[0].id}', true) RETURNING id`);
    binId = binResDb.rows[0].id;

    // Insert Stock (Opening)
    await pgClient.query(`INSERT INTO stock_balances (product_id, bin_id, current_quantity) VALUES ('${productId}', '${binId}', 500)`);
    await pgClient.query(`INSERT INTO stock_transactions (product_id, transaction_type, quantity, destination_bin_id, "referenceType", reference_id, created_by_id) VALUES ('${productId}', 'STOCK_IN', 500, '${binId}', 'SYSTEM', 'OPENING', '${EXISTING_USER_ID}')`);

    // Create Customer, PO, SC1, SC2
    const custRes = await fetch(`${BASE_URL}/customers`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${adminToken}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ code: `CUST-ADDL-${Date.now()}`, name: 'Addl Customer', isActive: true })
    });
    ...
    const poRes = await fetch(`${BASE_URL}/po`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${adminToken}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ poNumber: `PO-ADDL-${Date.now()}`, customerId, referenceDate: '2026-09-01' })
    });
    ...
    const scRes1 = await fetch(`${BASE_URL}/sc`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${adminToken}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ poId, scNumber: `SC-ADDL-${Date.now()}-1`, productName: 'Widget 1', targetQuantity: 10 })
    });
    ...
    const scRes2 = await fetch(`${BASE_URL}/sc`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${adminToken}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ poId, scNumber: `SC-ADDL-${Date.now()}-2`, productName: 'Widget 2', targetQuantity: 20 })
    });
    ...
    await fetch(`${BASE_URL}/rm/${rmReq2.id}/submit`, { method: 'POST', headers: { 'Authorization': `Bearer ${adminToken}` } });
    
    // Cheat: Map product and approve in DB to bypass Phase 12.4 complex UI flows
    await pgClient.query(`UPDATE rm_items SET mapped_product_id = '${productId}' WHERE id IN ('${rmItem1}', '${rmItem2}')`);
    await pgClient.query(`UPDATE sales_order_components SET status = 'IN_PRODUCTION' WHERE id IN ('${scId1}', '${scId2}')`);
    ...
    await fetch(`${BASE_URL}/production/receipt`, {
      ...
      body: JSON.stringify({ scId: scId1, items: [{ rmItemId: rmItem1, quantityReceived: 50 }] })
    });
    // Consume Material for SC1 (Consume 40)
    await fetch(`${BASE_URL}/production/consume`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${prodToken}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ scId: scId1, rmItemId: rmItem1, quantityConsumed: 40 })
    });
  ...
  it('ADDL_01: Should safely reject zero and negative quantities', async () => {
    let res = await fetch(`${BASE_URL}/additional-requests`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${prodToken}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ scId: scId1, items: [{ rmItemId: rmItem1, quantity: 0 }] })
    });
  ...
  it('ADDL_02: Should safely reject invalid UUIDs for SC and RM Item', async () => {
    let res = await fetch(`${BASE_URL}/additional-requests`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${prodToken}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ scId: 'not-a-uuid', items: [{ rmItemId: rmItem1, quantity: 10 }] })
    });
    expect(res.status).toBe(400);
    ...
    res = await fetch(`${BASE_URL}/additional-requests`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${prodToken}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ scId: scId1, items: [{ rmItemId: 'not-a-uuid', quantity: 10 }] })
    });
    expect(res.status).toBe(400);
  });
  ...
  it('ADDL_03: Should safely reject if rmItemId does not belong to the SC', async () => {
    const res = await fetch(`${BASE_URL}/additional-requests`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${prodToken}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ scId: scId1, items: [{ rmItemId: rmItem2, quantity: 10 }] }) // rmItem2 belongs to scId2
    });
    expect(res.status).toBe(400);
  ...
  it('ADDL_04: Should PREVENT mass assignment of status and quantityApproved', async () => {
    ...
    expect(res.status).toBe(400);
    const data = await res.json();
    expect(data.error.message).toEqual(expect.arrayContaining([
  ...
  it('ADDL_05: Should CREATE a valid Additional Request and strictly preserve Inventory & Accounting immutability', async () => {
    ...
    const accBeforeRes = await fetch(`${BASE_URL}/production/accounting/${scId1}`, { headers: { 'Authorization': `Bearer ${prodToken}` } });
    ...
    const accAfterRes = await fetch(`${BASE_URL}/production/accounting/${scId1}`, { headers: { 'Authorization': `Bearer ${prodToken}` } });
  ...
  it('ADDL_06: Should safely isolate multiple requests for the same SC', async () => {
    ...
    const res = await fetch(`${BASE_URL}/additional-requests`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${prodToken}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ scId: scId1, items: [{ rmItemId: rmItem1, quantity: 20 }] })
    });
    expect(res.status).toBe(201);

    const getRes = await fetch(`${BASE_URL}/additional-requests?scId=${scId1}`, {
  ...
  it('ADDL_07: Should completely isolate requests across different SCs and POs', async () => {
    const res2 = await fetch(`${BASE_URL}/additional-requests`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${prodToken}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ scId: scId2, items: [{ rmItemId: rmItem2, quantity: 50 }] })
    });
    expect(res2.status).toBe(201);

    const getRes1 = await fetch(`${BASE_URL}/additional-requests?scId=${scId1}`, { headers: { 'Authorization': `Bearer ${prodToken}` } });
    const list1 = await getRes1.json();
    
    const getRes2 = await fetch(`${BASE_URL}/additional-requests?scId=${scId2}`, { headers: { 'Authorization': `Bearer ${prodToken}` } });
