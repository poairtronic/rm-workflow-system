...
describe('Phase 12.2 - SC Real HTTP / Real DB Independence Verification', () => {
  beforeAll(async () => {
    ...
    const custRes = await fetch(`${BASE_URL}/customers`, {
      ...
      body: JSON.stringify({ name: 'SC Test Customer', code: `SC-CUST-${Date.now()}`, isActive: true })
    });
  ...
  it('SC_CREATE_02: Should create SC001, SC002 under PO-1 and SC003 under PO-2 successfully', async () => {
    const p1 = await fetch(`${BASE_URL}/sc`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ poId: poId1, scNumber: `SC001`, productName: 'Widget A', targetQuantity: 10 })
    });
    ...
    const p2 = await fetch(`${BASE_URL}/sc`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ poId: poId1, scNumber: `SC002`, productName: 'Widget B', targetQuantity: 20 })
    });
    ...
    const p3 = await fetch(`${BASE_URL}/sc`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ poId: poId2, scNumber: `SC001`, productName: 'Widget C', targetQuantity: 15 }) // Same SC number, different PO
    });
  ...
  it('SC_ISOLATION_01: Completing SC001 does not affect SC002', async () => {
    ...
    const sc2Check = await fetch(`${BASE_URL}/sc/${scId2}`, { headers: { 'Authorization': `Bearer ${token}` } });
  ...
  it('SC_VALIDATION_01: Should prevent duplicate SC numbers under the SAME PO', async () => {
    const dupRes = await fetch(`${BASE_URL}/sc`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
      body: JSON.stringify({ poId: poId1, scNumber: `SC002`, productName: 'Widget D', targetQuantity: 1 })
    });
  ...
  it('SC_LIST_01: FindAll properly filters by PO ID', async () => {
    const res = await fetch(`${BASE_URL}/sc?poId=${poId1}`, { headers: { 'Authorization': `Bearer ${token}` } });
