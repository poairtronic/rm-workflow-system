...
describe('Phase 12.8 - Production Return & Stores Acknowledgement', () => {
  beforeAll(async () => {
    ...
    const p1 = await pgClient.query(
      `INSERT INTO products (name, family_id, minimum_inventory) VALUES ('Product Return ' || gen_random_uuid()::text, '${fam.rows[0].id}', 0) RETURNING id`,
    );
  ...
  it('RETURN_01: Should safely reject negative return', async () => {
    const res = await fetch(`${BASE_URL}/production/return`, {
      ...
      body: JSON.stringify({
        scId,
        items: [{ rmItemId: rmItem1, quantityReturned: -5 }],
      }),
    });
  ...
  it('RETURN_02: Should successfully create pending return WITHOUT stock mutation', async () => {
    ...
    const txBefore = await pgClient.query(
      `SELECT COUNT(*) as c FROM stock_transactions WHERE product_id = '${productId}' AND destination_bin_id = '${binId}'`,
    );
    ...
    const res = await fetch(`${BASE_URL}/production/return`, {
      ...
      body: JSON.stringify({
        scId,
        items: [{ rmItemId: rmItem1, quantityReturned: 15 }],
      }),
    });
  ...
  it('RETURN_03: Should reject over-return (preventing cumulative over-return)', async () => {
    ...
    const res = await fetch(`${BASE_URL}/production/return`, {
      ...
      body: JSON.stringify({
        scId,
        items: [{ rmItemId: rmItem1, quantityReturned: 16 }],
      }),
    });
    expect(res.status).toBe(400);
  });
  ...
  it('RETURN_05: Should allow Stores to ACKNOWLEDGE the return and ATOMICALLY increase stock', async () => {
    ...
    const res = await fetch(
      `${BASE_URL}/production/return/${returnId}/verify`,
      {
        method: 'POST',
        headers: {
    ...
    expect(Number(stockAfter.rows[0].current_quantity)).toBe(
      Number(stockBefore.rows[0].current_quantity) + 15,
    );
    // It should insert exactly 1 transaction
    expect(Number(txAfter.rows[0].c)).toBe(Number(txBefore.rows[0].c) + 1);

    const txTypeRes = await pgClient.query(
      `SELECT transaction_type, quantity, reference_id FROM stock_transactions WHERE "referenceType" = 'MATERIAL_RETURN' AND reference_id = '${returnId}'`,
    );
  ...
  it('RETURN_07: Concurrency - Should enforce total returned limit under concurrent load', async () => {
    ...
    const req1 = fetch(`${BASE_URL}/production/return`, {
      ...
      body: JSON.stringify({
        scId,
        items: [{ rmItemId: rmItem1, quantityReturned: 10 }],
      }),
    });
