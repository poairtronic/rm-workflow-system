...
describe('Inventory DTO Validation', () => {
  ...
  beforeEach(() => {
    target = new ValidationPipe({ transform: true, whitelist: true, forbidNonWhitelisted: true });
  });
  ...
  describe('CreateStockInDto', () => {
    it('should reject negative quantities', async () => {
      ...
      await expect(target.transform(dto, { type: 'body', metatype: CreateStockInDto }))
    ...
    it('should accept valid decimal quantities', async () => {
      ...
      const result = await target.transform(dto, { type: 'body', metatype: CreateStockInDto });
  ...
  describe('GetTransactionFilterDto', () => {
    it('should set default pagination values', async () => {
      const dto = {};
      const result = await target.transform(dto, { type: 'query', metatype: GetTransactionFilterDto });
    ...
    it('should reject invalid page sizes', async () => {
      ...
      await expect(target.transform(dto, { type: 'query', metatype: GetTransactionFilterDto }))
        .rejects.toThrow('Bad Request Exception');
    });
