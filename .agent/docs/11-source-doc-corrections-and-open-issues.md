...
§22 states the rule _"unless the system is dealing with a separately defined additional transaction"_ but never defines what that separately-defined transaction is, or how it's exempted from the check. Likely intent: exceptions (wastage/damage/error) are tracked as their own quantity bucket and should be included in the ceiling check too, i.e. the real rule should probably be:
...
## 3. Whether Senior Designer can _create_ an RM is unresolved in the source itself

- §37 (Permission Matrix) marks "Create RM" for Senior Designer with `✓*` and a footnote: _"subject to your final business rule."_
